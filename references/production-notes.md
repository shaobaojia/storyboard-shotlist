...
## 2026-07-17: Seedance 角度控制实测 + 管线闭环

### 核心发现：空间锚文本不控制 Seedance 角度

**实测结论：** Seedance 的空间理解锚定在场景图自身的角度上。文本中的「贴地低角度」「相机在X右侧」「空间锚：男人站在护栏边」等空间位置描述，视频模型基本不服从——它始终以场景图的角度为基准生成。把场景图的角度找对，问题解决七七八八。

**新管线（三步）：**
1. **场景设定图：** 以全景为主，含正反打对向两个角度
2. **视频抽卡定人物位置：** 用视频模型（快乐小马 1.1 / Seedance）在场景图上抽卡，确定人物在空间中的位置
3. **摇出各机位角度帧：** 用视频模型从定位视频中摇出各机位角度的帧——同角度的镜头共用一张帧

**影响：** 从此抽卡只抽角色表演——角度是确定性的了。

### 对提示词模块的影响

- 空间锚文本从视频提示词中**完全删除**——其内容上移为机位帧生产阶段的规格
- 拍摄方式的机位方位描述从「相机在X右侧」收窄为「基于帧的微调」（略高/略低/微俯/微仰）
- 主光方位保留——实测：完全不写灯光时角色出现塑料感补光；灯光行的作用变为「把角色光锚定到帧内 practicals」
- 画面呈现改名为画面动态——职责收窄为「只写布局的时序变化」，静态构图不再重复描述（帧已承载）

### 场景图加特写效果

实测意外发现：把设定图里加上特写部分，效果出乎意料地好。
